    ...
    try:
        ...
        for job in job_data:
            ...
            for emp in employee_data:
                emp_dict = dict(zip(emp_columns, emp))
                if emp_dict['id'] in assigned_employees:
                    continue

                match_score = 0
                for skill in job_columns:
                    if skill in ["job_id", "job_name"]:
                        continue
                    if skill in emp_columns and emp_dict.get(skill, 0) >= job_dict.get(skill, 0):
                        match_score += 1
            ...
            if best_match:
                assigned_employees.add(best_match['id'])
                delegated_tasks.append({
                    "employee_name": best_match["name"],
                    "job_name": job_dict["job_name"]
                })

                # Insert into delegated_tasks table
                today = datetime.date.today().isoformat()
                cursor.execute("""
                    INSERT INTO delegated_tasks (name, delegated_job, date)
                    VALUES (%s, %s, %s)
                """, (best_match["name"], job_dict["job_name"], today))
                conn.commit()
        ...
        history_path = os.path.join(history_folder, f"delegation_{today}.json")
        with open(history_path, "w") as f:
            json.dump({"date": today, "tasks": delegated_tasks}, f, indent=4)

        # Keep only last 15 history files
